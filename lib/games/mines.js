import { MAX_BET_AMOUNT, MINES_BASE_MULTIPLIER } from './constants'
...
export async function revealMinesCell(ctx, { sessionId, cellIndex }) {
  ...
  if (session.game_type !== 'mines') throw new Error('Invalid game session')

  const state = session.metadata?.state || session.metadata

  if (!state || !Number.isInteger(cellIndex) || cellIndex < 0 || cellIndex >= state.gridSize) {
