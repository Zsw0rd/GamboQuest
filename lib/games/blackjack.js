...
async function settleBlackjack(ctx, sessionId, state, result) {
  let payout = 0
  ...
  const balance = payout > 0
    ? await adjustWallet(ctx.userId, ctx.guestToken, payout, 'blackjack', {
      action: 'settle',
      result,
      sessionId,
    })
    : await getWalletBalance(ctx.userId, ctx.guestToken)
...
export async function startBlackjack(ctx, bet) {
  if (!Number.isSafeInteger(bet) || bet <= 0 || bet > MAX_BET_AMOUNT) throw new Error('Invalid bet amount')
  ...
  if (playerValue === 21 || dealerValue === 21) {
    const result = playerValue === 21 && dealerValue === 21
      ? 'tie'
      : playerValue === 21
        ? 'blackjack'
        : 'lose'
...
export async function hitBlackjack(ctx, { sessionId }) {
  ...
  if (session.game_type !== 'blackjack') throw new Error('Invalid game session')

  const state = session.metadata?.state || session.metadata

  if (state.finished) throw new Error('Game already finished')
  if (handValue(state.playerHand) >= 21) throw new Error('Invalid hit: already at 21')
