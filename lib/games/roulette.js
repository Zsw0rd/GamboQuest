import {
  MAX_ROULETTE_TOTAL_BET,
  RED_NUMBERS,
  ROULETTE_WHEEL_ORDER,
  VALID_ROULETTE_SPOTS,
} from './constants'
...
export async function spinRoulette(ctx, bets) {
  ...
  for (const [spot, amount] of Object.entries(bets)) {
    if (!VALID_ROULETTE_SPOTS.has(spot)) {
      throw new Error('Invalid roulette spot')
    }
    if (!Number.isSafeInteger(amount) || amount <= 0) {
      throw new Error('Invalid bet amount')
    }
    totalBet += amount
    if (totalBet > MAX_ROULETTE_TOTAL_BET) {
      throw new Error(`Roulette bets are limited to ${MAX_ROULETTE_TOTAL_BET} GQC per spin`)
    }
  }
  ...
  for (const [spot, amount] of Object.entries(bets)) {
    const payout = calcSpotPayout(spot, winningNum, amount)
    totalWon += payout
    settledBets[spot] = {
      amount,
      payout,
